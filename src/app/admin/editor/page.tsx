...
export default function NewEditorPage() {
  ...
  const autoSave = useCallback(async () => {
    ...
    const payload = { ...form, slug, status: "DRAFT" as const };

    try {
      if (draftId) {
        await fetch(`/api/articles/${draftId}`, {
          method: "PATCH",
  ...
  async function handleSubmit(publishStatus: "DRAFT" | "PUBLISHED") {
    ...
    const slug = form.slug || slugify(form.title);
    const payload = { ...form, slug, status: publishStatus };

    try {
      let res: Response;
      if (draftId) {
        res = await fetch(`/api/articles/${draftId}`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(payload),
        });
  ...
  return (
    <div className="p-8 max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-serif font-bold text-ink">Artikel Baru</h1>
          {autoSaved && (
